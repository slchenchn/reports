...
2. 限流不依赖预先压测得到的固定并发阈值，而是基于实时观测的 TTFT、TPOT、latency 等指标动态调整限流阈值
...
| 指标     | 异常阈值 | 依据                                                                        |
| -------- | -------- | --------------------------------------------------------------------------- |
| p95 TTFT | > 20s    | 超过这个量级，说明首 token 等待已经明显恶化                                |
| p95 TPOT | > 200ms  | 长输出场景下，体感延迟已经会明显变差                                        |

为什么用 p95 而不是均值：均值对少量极端值不敏感，p95 更容易捕捉“已经有不少请求开始变慢”的信号
...
也就是说，恢复阈值是触发阈值的约 50%，且需要稳定保持 30 秒。这保证系统真正恢复健康后才重新放流，而不是刚从过载中喘过气就被新流量打回去
...
| TTFT 恢复阈值 | 10s           | 触发阈值的 50%                      |
| TPOT 恢复阈值 | 100ms         | 触发阈值的 50%                      |
